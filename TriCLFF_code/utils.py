...
from metrics import eval_mclust_ari,res_search_fixed_clus
...
def mclust_R(adata, num_cluster, modelNames='EEE', used_obsm='z', random_seed=2020):
    """\
...
def get_predicted_results(dataset, name, path, z, adata=None):

    if dataset=="MouseOlfactoryBulb":
        
        root_path = "/root/data/MouseOlfactoryBulb"
        ...
        silhouette_score = metrics.silhouette_score(adata.obsm['z'], adata.obs['mclust'].tolist())
        print('===== Project: {} silhouette_score: {:.3f}'.format('TriCLFF-Stereoseq', silhouette_score))
        plt.rcParams["figure.figsize"] = (5, 5)
        sc.pl.embedding(adata, basis="spatial", color="mclust",s=30, show=False, title=['TriCLFF(SC=%.3f)'%silhouette_score])
        plt.axis('off')
        plt.savefig('./stereoseq_new_7_clusters.pdf', bbox_inches='tight', dpi=600)
        adata.write(os.path.join('./', f'Stereoseq_adata_new222.h5ad'), compression="gzip")
        return silhouette_score, z
        
    elif dataset=="SpatialLIBD" or dataset== "MouseBrain" or dataset== "BreastCancer":
        ...
        adata = mclust_R(adata, used_obsm='z', num_cluster=n_clusters)
        obs_df = adata.obs.dropna()
        ARI = adjusted_rand_score(obs_df['mclust'], label)
        print('Adjusted rand index = %.3f' %ARI)
        refined_preds = obs_df['mclust']
