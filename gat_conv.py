...
class GATConv(MessagePassing):
    ...
    def __init__(self, in_channels: Union[int, Tuple[int, int]],
                 ...
                 negative_slope: float = 0.2, dropout: float = 0.3,
    ...
    def forward(self, x: Union[Tensor, OptPairTensor], edge_index: Adj,
        ...
        self.att_src = self.att_src.to(device)
        self.att_dst = self.att_dst.to(device)
        # We first transform the input node features. If a tuple is passed, we transform source and target node features via separate weights:
